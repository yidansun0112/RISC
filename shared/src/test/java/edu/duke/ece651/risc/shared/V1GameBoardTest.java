...
public class V1GameBoardTest {
  ...
  public void test_gameboard2() {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      assertEquals(t.get(i).getDefendUnitAmount(), -1);
    }
    ...
    for (int i = 3; i < 6; i++) {
      assertEquals(t.get(i).getOwner(), 1);
      assertEquals(t.get(i).getDefendUnitAmount(), 0);
    }
    ...
    assertEquals(terr.getDefendUnitAmount(), 0);
    b.addOwnUnits(0, 3);
    assertEquals(terr.getDefendUnitAmount(), 3);
    ...
    b.addOwnUnits(0, 2);
    assertEquals(terr.getDefendUnitAmount(), 5);
    b.updateAllPrevDefender(); // update
    b.removeUnits(0, 1);
    assertEquals(terr.getDefendUnitAmount(), 4);
