 ...
 * 
 * It handles for each player to pick territory, deploy units, and receive
 * orders from player. When a player wins, it prompts to ask the player to watch
 * or quit the game. This thread will end when somebody wins.
 * 
...
public class GameHostThread<T> extends Thread {
   ...
   * 
   * Send board view to player and receive choice from player. Then call
   ...
   * 
   * Send self group info to player, receive deployment from player. If legal,
   * send legal info. If illegal, send illegal info Continue send group info and
